...
import PrevievVideo from './PrevievVideo';
...
export default function WeddingCardEditor() {
  ...
  const [savedSmallVideos, setSavedSmallVideos] = useState({});
  ...
  const [showVideoUploadOptions, setShowVideoUploadOptions] = useState(false);
  ...
  const { videos: initialVideos = [] } = location.state || {}; // Default to an empty array if videos are not present
  const [selectedField, setSelectedField] = useState(null);
  // Fetch savedPages from localStorage and set the text fields based on currentImageIndex
  useEffect(() => {
    if (initialVideos.length > 0) {
      setVideos(initialVideos); // Set initial videos from location state
      setSelectedField(null);
    }
  }, [initialVideos]);
  const handleCopyVideo = (index) => {
    if (index !== null) {
      const newVideo = videos[index]; // Assuming you want to copy the same video
  ...
  useEffect(() => {
    ...
    const savedSmallVideosFromStorage = JSON.parse(localStorage.getItem('savedSmallVideos')) || {};
    ...
    setSavedSmallVideos(savedSmallVideosFromStorage);
    setSavedStickers(savedStickersFromStorage);  // Store stickers in state

    // Load text fields, small videos, and stickers for the current video index
    if (Array.isArray(savedPagesFromStorage[currentVideoIndex])) {
      setTextFields(savedPagesFromStorage[currentVideoIndex]);
    }

    if (Array.isArray(savedSmallVideosFromStorage[currentVideoIndex])) {
      setSmallVideos(savedSmallVideosFromStorage[currentVideoIndex]);
    }

    if (Array.isArray(savedStickersFromStorage[currentVideoIndex])) {
      setStickers(savedStickersFromStorage[currentVideoIndex]);
    }
  }, [currentVideoIndex]);

  const handleDeleteVideo = (videoId) => {
    setSmallVideos((prev) => prev.filter((video) => video.id !== videoId));
  };
  ...
  const handleDownloadPDF = () => {
    generateVideo(); // Call the PDF generation function
  };
  ...
  function generateVideo() {
    ...
    video.src = videoUrl;
    video.muted = true; // Mute the original video to prevent audio conflicts
    video.crossOrigin = "anonymous"; // Important for accessing video data from different domains
    video.autoplay = true;
    video.loop = true;
    ...
    video.onloadeddata = () => {
        ...
        mediaRecorder.onstop = () => {
            ...
            link.href = url;
            link.download = 'generated_video.webm';
        ...
        const loadImages = (images) => {
            return Promise.all(images.map(({ src }) => {
                return new Promise((resolve, reject) => {
                    const img = new Image();
                    img.crossOrigin = "anonymous"; // Important for images from different domains
                    img.onload = () => resolve(img);
                    img.onerror = (err) => reject(new Error(`Image failed to load: ${src} - ${err}`));
                    img.src = src;
                });
            }));
        };

        Promise.all([loadImages(smallImages), loadImages(stickers)]).then((loadedImages) => {
            const loadedSmallImages = loadedImages[0];
            const loadedStickers = loadedImages[1];

            const draw = () => {
                ctx.drawImage(video, 0, 0, canvas.width, canvas.height);

                textFields.forEach(({ text, x, y, font, size, color, angle }) => {
                    ctx.save();
                ...
                // Draw small images (using loaded images)
                smallImages.forEach((image, index) => {
                    ctx.drawImage(loadedSmallImages[index], image.x, image.y, image.width, image.height);
                });

                // Draw stickers (using loaded images)
                stickers.forEach((sticker, index) => {
                    ctx.save();
                    ctx.translate(sticker.x + sticker.width / 2, sticker.y + sticker.height / 2);
                    ctx.rotate(sticker.rotation * Math.PI / 180);
                    ctx.drawImage(loadedStickers[index], -sticker.width / 2, -sticker.height / 2, sticker.width, sticker.height);
                    ctx.restore();
                });
    ...
    video.onerror = (error) => console.error('Video failed to load:', error);
}
  ...
  const handleAddSmallVideo = (src) => {
    const newVideo = {
      id: `smallVideo${Date.now()}`,
      src,
    ...
    setSmallVideos((prevVideos) => [...prevVideos, newVideo]);
  };

  const handleSaveChanges = () => {
    const savedPages = JSON.parse(localStorage.getItem('savedPages')) || {};
    savedPages[currentVideoIndex] = textFields;
    localStorage.setItem('savedPages', JSON.stringify(savedPages));

    const savedSmallVideos = JSON.parse(localStorage.getItem('savedSmallVideos')) || {};
    savedSmallVideos[currentVideoIndex] = smallVideos;
    localStorage.setItem('savedSmallVideos', JSON.stringify(savedSmallVideos));

    const savedStickers = JSON.parse(localStorage.getItem('savedStickers')) || {};
    savedStickers[currentVideoIndex] = stickers; // Save stickers data to localStorage
  ...
  return (
    <div className="container mx-auto  md:bg-gradient-to-br">
      ...
      <div className="flex w-full relative">
        ...
        <div className="hidden md:block w-48 lg:w-56  flex-col gap-6 md:p-4 lg:p-6 h-screen overflow-y-auto bg-gray-100 border-r border-gray-300 shadow-md">
          {/* Title */}
          ...
          {videos.length > 0 ? (
            videos.map((vid, index) => (
              <div key={index} className={`flex flex-col items-center gap-2 p-2 border rounded-lg transition-all duration-300 ${selectedVideoIndex === index ? 'bg-[#FDF5E6] shadow-md border-[#AF7D32]' : 'bg-white border-gray-200 hover:shadow-lg hover:border-gray-300'}`}>
                <video src={vid} className={`w-full rounded-md object-cover cursor-pointer transform transition duration-300 ${selectedVideoIndex === index ? 'scale-105' : 'filter blur-sm hover:blur-none'}`} onClick={() => handleThumbnailClick(index)} controls />
                <span className={`mt-1 text-center text-sm font-medium ${selectedVideoIndex === index ? 'text-[#AF7D32]' : 'text-gray-600'}`}>
                  Video {index + 1}
                </span>
              </div>
            ))
          ) : (
            <p className="text-center text-gray-500 mt-6">No videos available.</p>
          )}
        ...
        {isDownloading && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="text-center">
              ...
              {/* Loading Text */}
              <p className="text-white text-lg font-semibold mt-4">Your Video is being prepared, please wait...</p>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="relative bg-white p-6 rounded-lg shadow-lg flex flex-col">
              ...
              <div className="gap-4 flex flex-col overflow-y-auto" style={{ height: "500px" }}>
                {videos.map((vid, index) => (
                  <div key={index} className="flex items-center w-full gap-4 md:p-2 border-gray-500 lg:p-3 border-b border-t border-l border-r border-gray pb-3" draggable onDragStart={() => handleDragStart(index)} onDragOver={handleDragOver} onDrop={() => handleDrop(index)}>
                    <button onClick={() => { /* Add functionality to rearrange videos if needed */ }} className="mt-4 px-6 py-2 bg-[#AF7D32] text-white cursor-move rounded hover:bg-[#643C28] transition flex items-center gap-2" style={{ cursor: "move" }}>
                      <FaArrowsAlt size={20} />
                    </button>
                    <video src={vid} className="w-36 object-contain cursor-move transition duration-300 border rounded-md h-3/3" controls />
                    <div className="flex gap-3">
                      <button onClick={() => handleCopyVideo(index)} className="text-blue-500 hover:text-blue-600 transition ml-10" title="Duplicate">
                        <FaPlus />
                      </button>
                      <button onClick={() => handleDeleteVideo(index)} className="text-[#AF7D32] hover:text-red-600 transition ml-10" title="Delete">
                        <FaTrash />
                      </button>
                    </div>
                  </div>
              ...
              <button onClick={() => setIsCustomizeModalOpen(false)} className="mt-4 px-6 py-2 bg-[#AF7D32] text-white rounded hover:bg-red-500 transition">
                Close
              </button>
            </div>
          </div>
        ...
        >
          <div className="flex flex-col 2xl:mb-6 md:ml-0 sm:ml-0 xl:mb-6">
            {/* Image Container */}
            <div className="flex flex-col 2xl:mb-6 md:ml-0 sm:ml-0 xl:mb-6 xl:ml-56 md:mr-4 mr-8">
              <div className="relative md:w-3/5 lg:w-80 lg:h-112 flex items-center mr-8 rounded-lg shadow-md border border-gray-200 bg-gray-50 overflow-hidden">
                ...
                <div className="absolute top-1/2 left-4 transform -translate-y-1/2">
                  <button onClick={handlePreviousVideo} className="p-3 bg-white text-gray-700 rounded-full shadow hover:bg-gray-100 hover:text-gray-900 transition duration-300">
                    <FaArrowLeft size={20} />
                  </button>
                </div>
                <div className="absolute top-1/2 right-4 transform -translate-y-1/2">
                  <button onClick={handleNextVideo} className="p-3 bg-white text-gray-700 rounded-full shadow hover:bg-gray-100 hover:text-gray-900 transition duration-300">
                ...
                {textFields.map(({ id, text, x, y, size, font, fontColor, angle, isBold, isItalic, textAlign, lineHeight, letterSpacing, isUppercase, isLowercase, curveValue }) => (
                  <div key={id} className="absolute" style={{
                    ...
                    {selectedField === id && (
                      <div onMouseDown={(e) => handleResizeMouseDown(e, id)} className="absolute right-0 bottom-0 w-6 h-6 cursor-se-resize border-2 border-blue-500 rounded-full flex justify-center items-center" style={{ transform: "translate(50%, 50%)" }}>
                    ...
                    {selectedField === id && (
                      <button onClick={(e) => {
                        e.stopPropagation();
                        handleDelete(id);
                      }} className="absolute top-0 right-0 w-6 h-6 rounded-full shadow bg-white border-2 border-blue-500 flex justify-center items-center" style={{
                    ...
                    {selectedField === id && (
                      <div onMouseDown={(e) => handleRotateMouseDown(e, id)} className="absolute top-0 left-0 w-6 h-6 cursor-pointer bg-white rounded-full flex justify-center items-center" style={{
                        transform: "translate(-50%, -50%)",
                        zIndex: 20,
                      }}>
              ...
              <div className="flex lg:gap-4 md:mt-4 lg:mt-1 gap-4 sm:mt-4 mt-6">
                <button onClick={handleSaveChanges} className="flex items-center justify-center gap-2 px-10 py-3 bg-[#AF7D32] text-white font-medium rounded-lg shadow-lg hover:bg-[#643C28] transform hover:scale-105 transition-all duration-300">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5v6.75a2.25 2.25 0 01-2.25 2.25h-10.5a2.25 2.25 0 01-2.25-2.25V10.5m15 0v-3.75a2.25 2.25 0 00-2.25-2.25h-10.5a2.25 2.25 0 00-2.25 2.25V10.5m15 0h-15" />
                  </svg>
                  <span>Save</span>
                </button>

                <button onClick={handlePreview} className="flex items-center justify-center gap-2 px-10 py-3 bg-[#AF7D32] text-white font-medium rounded-lg shadow-lg hover:bg-[#643C28] transform hover:scale-105 transition-all duration-300">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.684c.046-.08.093-.16.142-.238A11.284 11.284 0 0112 4.5c3.9 0 7.345 1.882 9.878 4.946a11.422 11.422 0 011.142 1.52c.03.05.058.1.086.15m-19.126.568a11.422 11.422 0 01.086-.15m0 0a11.422 11.422 0 011.142-1.52C4.655 6.382 8.1 4.5 12 4.5c3.9 0 7.345 1.882 9.878 4.946.049.078.096.158.142.238m-19.126.568C2.292 9.77 2.25 9.888 2.25
10.5c0 .612.042.73.116.848.235.376.595.91 1.142 1.52 2.533 3.064 5.978 4.946 9.878 4.946 3.9 0 7.345-1.882 9.878-4.946a11.283 11.283 0 001.142-1.52c.074-.118.116-.236.116-.848 0-.612-.042-.73-.116-.848a11.283 11.283 0 00-1.142-1.52C19.345 6.382 15.9 4.5 12 4.5c-3.9 0-7.345 1.882-9.878 4.946a11.422 11.422 0 00-1.142 1.52z" />
          ...
          {showPreview && (
            <PrevievVideo
              videos={smallImages} // Pass the small videos to the preview
              textFields={textFields} // Pass the text fields to the preview
              stickers={stickers} // Pass stickers to the preview
              currentVideo={videoUrl} // Pass the current video URL
