      ...
      <div className="flex sm:flex-col gap-2 sm:gap-3  pb-2 sm:pb-0 w-full sm:w-auto">
        {filteredProduct.image.map((img, index) => (
          <img
            ...
            className={`flex-shrink-0 cursor-pointer w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg border-2 transition-all duration-300 hover:scale-105 ${
              ...
              <div className="border rounded-lg bg-white shadow-lg overflow-hidden">
                {[1, 2, 3, 4].map((num) => (
                  <button
                    ...
                    className={`w-full px-4 py-2.5 text-left text-gray-700 hover:bg-gray-50 active:bg-gray-100 transition-colors ${
                      quantity === num ? 'bg-gray-50' : ''
                    }`}
                  >
                    {num}
                  </button>
